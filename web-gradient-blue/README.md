# Web Gradient Blue Snapshot (Next.js + Sanity)

This app reads site content from Sanity and is intended to deploy on Vercel.
It is a standalone snapshot extracted from commit `025303a` (March 15, 2026), which included the blue-gradient styling.
...
2. Start this Next.js frontend:
   ...
   ```bash
   cd web-gradient-blue
   cp .env.example .env.local
   npm run dev
   ```

   Optional overrides:
   - `WEB_GRADIENT_HOST` (default `127.0.0.1`)
   - `WEB_GRADIENT_PORT` (default `3001`)
...
Set these in `web-gradient-blue/.env.local` locally and in Vercel Project Settings for production:
...
1. Ensure the Vercel project deploys the `web-gradient-blue/` directory.
